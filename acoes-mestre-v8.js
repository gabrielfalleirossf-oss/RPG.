window.iniciarAcoesMestreV8=async function(){
 ...
 async function mostrarConfronto(d,selecionados){
  ...
  area.querySelector("#iniciar-confronto").onclick=async()=>{const escolhido=lista.querySelector("input:checked");if(!escolhido){status(d,"Escolha um oponente.",true);return}const botao=area.querySelector("#iniciar-confronto");botao.disabled=true;status(d,"Preparando confronto...");const tipo=escolhido.dataset.tipo,{data:id,error}=await auth.cliente.rpc("iniciar_confronto_mestre",{ficha_principal:principal.id,tipo_oponente:tipo,oponente_id:escolhido.value});if(error){status(d,error.message,true);botao.disabled=false;return}const base=tipo==="npc"?"../npc/ficha/index.html":"../confronto/index.html",url=base+"?campanha="+encodeURIComponent(campanha)+(tipo==="npc"?"&id="+encodeURIComponent(escolhido.value):"")+"&confronto="+id;window.open(url,"_blank","noopener");status(d,"Confronto iniciado! A ficha foi aberta em uma nova guia.");setTimeout(()=>d.close(),900)};
 }
