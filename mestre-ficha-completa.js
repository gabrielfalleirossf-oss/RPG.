...
window.mestreOrigens=mestreOrigens;
...
function montarPericiasMestre(){const lista=document.querySelector("#lista-pericias");lista.innerHTML="";mestrePericias.forEach(([nome,atr])=>{const origem=Number(mestreOrigens[mestrePersonagem?.origem]?.pericias?.[nome])||0,bonus=mestrePersonagem?.bonus_pericias&&Object.prototype.hasOwnProperty.call(mestrePersonagem.bonus_pericias,nome)?Number(mestrePersonagem.bonus_pericias[nome])||0:origem,linha=document.createElement("button");linha.type="button";linha.className="pericia";linha.dataset.pericia=nome;linha.innerHTML="<span></span><span></span><span></span><span>0</span>";linha.children[0].textContent=nome;linha.children[1].textContent=`(${atr})`;linha.children[2].textContent=bonus>0?`+${bonus}`:String(bonus);linha.addEventListener("click",()=>{if(!document.body.classList.contains("edicao-direta-mestre"))rolarMestre(1,bonus,`${nome}: `)});lista.appendChild(linha)})}
...
function configurarInterfaceMestre(){
  ...
  const dialog=document.querySelector("#editor-mestre");document.querySelector("#fechar-editor").addEventListener("click",()=>dialog.close());document.querySelector("#cancelar-editor").addEventListener("click",()=>dialog.close());
...
async function carregarRecursosDaFichaMestre(){const estilo=document.createElement("link");estilo.rel="stylesheet";estilo.href="../../../ficha-recursos.css?v=49";document.head.appendChild(estilo);await new Promise((resolve,reject)=>{const script=document.createElement("script");script.src="../../../ficha-recursos.js?v=58";script.onload=resolve;script.onerror=reject;document.head.appendChild(script)});await window.iniciarRecursosFicha()}
